...
class Eventos():
    ...
    def Autor(self):
        ...
        try:
            msgBox = QMessageBox()
            msgBox.setIcon(QtWidgets.QMessageBox.Information)
            msgBox.setText("Nombre: Xoel Fernández Besada"
                           "\nFecha de Nacimiento: 24/11/1995")
            msgBox.setWindowTitle("Datos del autor")
            msgBox.exec()
        except Exception as error:
            print('Error en la ventana de Autor ', error)
    ...
    def Contacto(self):
        ...
        try:
            msgBox = QMessageBox()
            msgBox.setIcon(QtWidgets.QMessageBox.Information)
            msgBox.setText("Correo: xoel.fernandez.besada@gmail.com"
                           "\nTeléfono: +34 665 212 785")
            msgBox.setWindowTitle("Datos de contacto")
            msgBox.exec()
        except Exception as error:
            print('Error en la ventana de Contacto ', error)
    ...
    def Version(self):
        ...
        try:
            msgBox = QMessageBox()
            msgBox.setIcon(QtWidgets.QMessageBox.Information)
            msgBox.setText("Programa en fase Alpha"
                           "\nVersión: 0.7.4")
            msgBox.setWindowTitle("Datos de la versión")
            msgBox.exec()
        except Exception as error:
            print('Error en la ventana de Versión ', error)

    def Acercade(self):

        try:
            var.dlgacercade.show()
            if var.dlgacercade.exec():
                var.dlgacercade.hide()

        except Exception as error:
            print('Error en módulo Acerca de ', error)