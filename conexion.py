...
class Conexion:
    ...
    def cargaTabFacturasCliente(dni):
        """
        ...
        try:
            index = 0
            query = QtSql.QSqlQuery()
            query.prepare('select codigo, fechafac from facturas where dni=:dni order by date(fechafac) desc ')
            query.bindValue(':dni', dni)
            if query.exec_():
                while query.next():
                    codigo = query.value(0)
    ...
    def cargarLineasVenta(codfac):
        ...
        try:
            ...
            if not var.ui.txtDesc.text() == '':
                descuento_porcentaje = float(var.ui.txtDesc.text())
                print(descuento_porcentaje)
                descuento = (descuento_porcentaje * suma) / 100
                descuento_total = suma - descuento
                iva = descuento_total * 0.21
                total = descuento_total + iva
                var.ui.lblSubTotal.setText(str('{:.2f}'.format(round(suma, 2))) + '€')
                var.ui.lblDesc.setText(str('{:.2f}'.format(round(descuento, 2))) + '€')
